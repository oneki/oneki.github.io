...
:::caution
The `<App/>` component is not the same for apps built on top of **Create React App** or **Next.js**<br/>
...
### Parameters
  ...
  | store            | ReduxStore      | A standard Redux store, but created via the helper createReduxStore from onekijs<br/>The store must be created via this helper so onekijs can control it<br/><br/>**Defaults**: A store created by <strong>App</strong> (recommended) |
  ...
  | LoadingComponent | React Component | A component expected by <Suspense\> (used to display a loading indicator)<br/><br/>**Defaults**: `DefaultLoadingComponent` that displays "Loading..."                                                                   |
...
- **Settings**: the settings are defined in the file `src/settings.ts`
...
<p/>
  ...
  | Parameter        | Type            | Description                                                                                                                                                                                                                |
  | ---------------- | --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
  | settings         | object          | Settings is a object generally defined in the file src/settings.ts<br/>Data defined in settings.ts are available across the application and contains configuration data.<br/><br/>**Defaults**: {}                             |
...
:::
  ...
  height="450"
  modules={['/src/pages/_app.tsx','/src/settings.ts', '/src/pages/index.tsx']}
  />
